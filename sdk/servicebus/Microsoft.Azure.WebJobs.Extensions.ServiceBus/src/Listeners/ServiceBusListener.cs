...
namespace Microsoft.Azure.WebJobs.ServiceBus.Listeners
{
    internal sealed class ServiceBusListener : IListener, IScaleMonitorProvider
    {
        ...
        private int _sessionsMaxConcurrentCalls;
        ...
        private void UpdateConcurrency(object state)
        {
            ...
            try
            {
                if (_isSessionsEnabled)
                {
                    ...
                    int maxConcurrentSessions = Math.Min(sessionProcessor.MaxConcurrentSessions, currentConcurrency);
                    if (_serviceBusOptions.DynamicallyScaleSessionCallConcurrency)
                    {
                        if (maxConcurrentSessions != sessionProcessor.MaxConcurrentSessions || currentConcurrency != _sessionsMaxConcurrentCalls)
                        {
                            // TODO: waiting on the overload that allows us to specify a cross-session MaxConcurrentCalls
                            //sessionProcessor.UpdateConcurrency(maxConcurrentSessions, int.MaxValue, currentConcurrency);

                            _sessionsMaxConcurrentCalls = currentConcurrency;
                        }
                    }
                    else
                    {
                        if (maxConcurrentSessions != sessionProcessor.MaxConcurrentSessions)
                        {
                            // Per session concurrency is limited to 1. We only scale the number of concurrent sessions here
                            sessionProcessor.UpdateConcurrency(maxConcurrentSessions, sessionProcessor.MaxConcurrentCallsPerSession);
                        }
                    }
                }
                else
